...
function ProgressRow({ label, spent, total, remainingText, pct, over }: { label: string; spent: string; total: string; remainingText: string; pct: number; over?: boolean }) {
  return (
    <div className="px-3 py-2.5 border-b border-border last:border-0">
      ...
      <div className="flex justify-between mt-1 text-[10px] text-muted-foreground">
        <span>{pct}%</span>
        <span className={over ? 'text-destructive' : undefined}>{remainingText}</span>
      </div>
    </div>
  );
}

function pieGradient(segments: { pct: number; color: string }[]) {
  let acc = 0;
...
function useScreens(): Screen[] {
  const t = useTranslations('Landing.screens');
  const tDash = useTranslations('Dashboard');
  const tCharts = useTranslations('Charts');
  const tBudget = useTranslations('BudgetMonth');
  const tLiab = useTranslations('Liabilities');
  const tInv = useTranslations('Investments');
  const tInvCat = useTranslations('InvestmentCategories');
  const tTrends = useTranslations('Trends');

  const PIE_SEGMENTS = [
    { label: t('categoryFood'), pct: 35, color: 'hsl(var(--chart-1))' },
    { label: t('categoryTransport'), pct: 22, color: 'hsl(var(--chart-2))' },
    { label: t('categoryEntertainment'), pct: 18, color: 'hsl(var(--chart-3))' },
    { label: t('categoryBills'), pct: 15, color: 'hsl(var(--chart-5))' },
    { label: t('categoryOther'), pct: 10, color: 'hsl(var(--chart-4))' },
  ];

  const BAR_MONTHS = [
    { m: t('monthApr'), income: 68, expense: 46 },
    { m: t('monthMay'), income: 54, expense: 50 },
    { m: t('monthJun'), income: 78, expense: 41 },
    { m: t('monthJul'), income: 64, expense: 57 },
  ];
  ...
  return [
      ...
      render: () => (
        <div className="space-y-3">
          <div className="grid grid-cols-5 gap-2">
            {statTile(tDash('income'), '8 200 zł', 'text-[#01581E]', 'bg-[#01581E]/10', TrendingUp)}
            {statTile(tDash('expense'), '5 430 zł', 'text-destructive', 'bg-destructive/10', TrendingDown)}
            {statTile(tDash('savings'), '2 770 zł', 'text-blue-500', 'bg-blue-500/10', Wallet)}
            {statTile(tDash('liabilities'), `3 ${tDash('liabilitiesActive')}`, 'text-amber-500', 'bg-amber-500/10', HandCoins)}
            {statTile(tDash('investments'), '42 300 zł', 'text-indigo-500', 'bg-indigo-500/10', LineChart)}
          </div>
          <div className="bg-card border border-border rounded-lg p-3">
            <div className="flex items-center justify-between mb-1.5">
              <p className="text-xs font-medium text-foreground">{tDash('budgetProgress')}</p>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] font-semibold text-foreground mb-2">{tCharts('incomeVsExpense')}</p>
            ...
            <div className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] font-semibold text-foreground mb-2">{tCharts('expenseStructure')}</p>
              <div className="flex items-center gap-3">
                <div className="relative w-14 h-14 shrink-0">
                  <div className="w-14 h-14 rounded-full" style={{ background: pieGradient(PIE_SEGMENTS) }} />
      ...
      render: () => (
        <div className="space-y-2.5">
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: tBudget('plan'), value: '2 600 zł' },
              { label: tBudget('actual'), value: '2 220 zł' },
              { label: tBudget('difference'), value: '+380 zł' },
            ].map((s) => (
          ...
          <div className="bg-card border border-border rounded-lg overflow-hidden">
            <ProgressRow label={t('categoryFood')} spent="850 zł" total="1 000 zł" pct={85} remainingText={tBudget('remaining', { amount: '150 zł' })} />
            <ProgressRow label={t('categoryTransport')} spent="320 zł" total="400 zł" pct={80} remainingText={tBudget('remaining', { amount: '80 zł' })} />
            <ProgressRow label={t('categoryEntertainment')} spent="290 zł" total="300 zł" pct={97} remainingText={tBudget('remaining', { amount: '10 zł' })} />
            <ProgressRow label={t('categoryBills')} spent="660 zł" total="650 zł" pct={100} over remainingText={tBudget('overBudget', { amount: '10 zł' })} />
          </div>
        </div>
      ),
    },
    {
      key: 'liabilities',
      label: t('tabs.liabilities'),
      icon: Gavel,
      path: 'app.usescrooge.com/liabilities',
      render: () => (
        <div className="space-y-2.5">
          {[
            { icon: HandCoins, name: t('liabilityMortgage'), type: tLiab('typeLoan'), remaining: '245 000 zł', total: '320 000 zł', pct: 23, rate: '1 850 zł', color: 'text-red-500' },
            { icon: Landmark, name: t('liabilityBankLoan'), type: tLiab('typeBankLoan'), remaining: '12 400 zł', total: '20 000 zł', pct: 38, rate: '620 zł', color: 'text-cyan-500' },
            { icon: CreditCard, name: t('liabilityCreditCard'), type: tLiab('typeCredit'), remaining: '2 100 zł', total: '5 000 zł', pct: 58, rate: null, color: 'text-orange-500' },
            { icon: RefreshCw, name: t('liabilitySubscriptions'), type: t('subscriptionsDetail'), remaining: '87 zł', total: '87 zł', pct: 100, rate: '87 zł', color: 'text-blue-500' },
          ].map((l) => (
            <div key={l.name} className="bg-card border border-border rounded-lg p-3">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-[9px] text-muted-foreground">{tLiab('of', { total: l.total })}</p>
                </div>
              </div>
              ...
              <div className="flex justify-between text-[9px] text-muted-foreground">
                <span>{tLiab('paidPercent', { percent: l.pct })}</span>
                <span>{t('paymentPrefix')} {l.rate ?? '—'}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      render: () => (
        <div className="space-y-2.5">
          <div className="bg-[#01581E] rounded-lg p-3.5 text-white">
            <p className="text-white/70 text-[10px] mb-1">{tInv('totalValue')}</p>
            <p className="text-xl font-bold">42 300 zł</p>
            <p className="text-white/60 text-[10px] mt-1">{tInv('positionsCount', { count: 4 })}</p>
          </div>
          <div className="grid grid-cols-2 gap-2.5">
            {[
              { name: t('investmentStocks'), category: tInvCat('stocks'), value: '12 400 zł' },
              { name: t('investmentEtf'), category: tInvCat('etf'), value: '15 800 zł' },
              { name: t('investmentCrypto'), category: tInvCat('cryptocurrencies'), value: '8 100 zł' },
              { name: t('investmentDeposit'), category: tInvCat('deposits'), value: '6 000 zł' },
            ].map((inv) => (
        ...
        return (
          <div className="space-y-2.5">
            <div className="flex gap-1 p-1 bg-muted rounded-lg w-fit">
              {[tTrends('metricExpense'), tTrends('metricIncome'), tTrends('metricSavings')].map((m, i) => (
            ...
            <div className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] font-semibold text-foreground mb-2">{t('trendsChartTitle')}</p>
              ...
              <table className="w-full text-[10px]">
                <thead>
                  <tr className="border-b border-border text-muted-foreground">
                    <th className="text-left font-medium py-1.5 px-3">{tTrends('colYear')}</th>
                    <th className="text-right font-medium py-1.5 px-3">{tTrends('colIncome')}</th>
                    <th className="text-right font-medium py-1.5 px-3">{tTrends('colExpense')}</th>
                    <th className="text-right font-medium py-1.5 px-3">{tTrends('colChange')}</th>
                  </tr>
                </thead>
