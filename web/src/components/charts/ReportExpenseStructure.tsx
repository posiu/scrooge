...
export function ReportExpenseStructure({ month }: { month: string }) {
  ...
  if (data.length === 0) return (
    <div className="h-64 flex items-center justify-center text-sm text-muted-foreground">{t('noExpensesSelectedMonth')}</div>
  );

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-foreground">{t('expenseStructure')}</h3>
        <span className="text-sm text-muted-foreground">{t('total')} <span className="font-medium text-foreground">{formatCurrency(total)}</span></span>
      </div>
