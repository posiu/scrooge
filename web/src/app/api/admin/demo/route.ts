...
export async function POST(req: NextRequest) {
  ...
  for (const cat of catDefs) {
    const [c] = await db.insert(categories).values({
      userId: user.id, ...cat, isSystem: false, isDemoData: true,
    }).returning();
...
export async function DELETE(req: NextRequest) {
  ...
  // Delete in FK-safe order (children first) — every table filtered by
  // isDemoData=true so this can never touch the user's real data.
  await db.delete(enforcementPayments).where(and(eq(enforcementPayments.userId, user.id), eq(enforcementPayments.isDemoData, true)));
  await db.delete(taxPayments).where(and(eq(taxPayments.userId, user.id), eq(taxPayments.isDemoData, true)));
  await db.delete(enforcementProceedings).where(and(eq(enforcementProceedings.userId, user.id), eq(enforcementProceedings.isDemoData, true)));
  await db.delete(taxes).where(and(eq(taxes.userId, user.id), eq(taxes.isDemoData, true)));
  await db.delete(transactions).where(and(eq(transactions.userId, user.id), eq(transactions.isDemoData, true)));
  await db.delete(categories).where(and(eq(categories.userId, user.id), eq(categories.isDemoData, true)));
  await db.delete(accounts).where(and(eq(accounts.userId, user.id), eq(accounts.isDemoData, true)));
