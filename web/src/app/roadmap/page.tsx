...
const STATUS_COLORS: Record<Status, { color: string; bg: string }> = {
  open:        { color: 'text-blue-600',   bg: 'bg-blue-500/10' },
  planned:     { color: 'text-amber-600',  bg: 'bg-amber-500/10' },
  in_progress: { color: 'text-[#01581E]',  bg: 'bg-[#01581E]/10' },
  done:        { color: 'text-[#01581E]',  bg: 'bg-[#01581E]/15' },
...
export default function RoadmapPage() {
  ...
  const statusLabel: Record<Status, string> = {
    open: t('statusOpen'), planned: t('statusPlanned'), in_progress: t('statusInProgress'),
    done: t('statusDone'), rejected: t('statusRejected'),
  };
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        >
          <Plus className="w-4 h-4" /> {t('submitIdea')}
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          {t('allCount', { count: requests.length })}
        </button>
        {(Object.keys(STATUS_COLORS) as Status[]).map((status) => (
          <button
            ...
            className={cn('px-3 py-1.5 rounded-lg text-xs font-medium transition-colors',
              filter === status ? `${STATUS_COLORS[status].bg} ${STATUS_COLORS[status].color}` : 'bg-muted text-muted-foreground hover:text-foreground'
            )}
          >
            {statusLabel[status]} {statusCounts[status] ? `(${statusCounts[status]})` : ''}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered
            ...
            .map((req) => {
              const meta = STATUS_COLORS[req.status];
                  ...
                  <div className="flex items-start gap-4 p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                        <span>{req.authorName ?? t('anonymous')}</span>
                        <span>·</span>
                        <span>{new Date(req.createdAt).toLocaleDateString(INTL_LOCALE[locale])}</span>
                        ...
                        <div className="mt-3 space-y-2">
                          ...
                          {req.adminNote && (
                            <div className="bg-[#01581E]/5 border border-[#01581E]/20 rounded-lg p-3">
                              <p className="text-xs font-medium text-[#01581E] mb-1">{t('adminResponse')}</p>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
        <div className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            <div>
              <label className="text-xs font-medium text-foreground block mb-1.5">{t('titleLabel')}</label>
              <input
                type="text"
                value={title}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs font-medium text-foreground block mb-1.5">{t('emailLabel')}</label>
                <input
                  type="email"
                  value={authorEmail}
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="submit" disabled={loading} className="flex-1 px-4 py-2 rounded-lg bg-[#01581E] text-white text-sm font-medium hover:bg-[#01581E]/90 transition-colors disabled:opacity-50">
                {loading ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : t('submit')}
              </button>
            </div>
          </form>
        </div>
